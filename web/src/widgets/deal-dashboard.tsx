...
function InvestmentMemo({ slides, decision, avgScore, rubric, decisionGate, dealInput }: {
  ...
  const triggerDownload = (content: string, filename: string, mimeType: string) => {
    // Use top-level window to escape iframe sandbox
    ...
    a.click();
    setTimeout(() => { w.document.body.removeChild(a); URL.revokeObjectURL(url); }, 200);
  };
...
function InvestorProfileBadge({ firmType, aum, dealId, onSwitch }: {
  firmType?: string;
  aum?: string;
  dealId: string;
  ...
  const handleApply = async (rerun: boolean) => {
    ...
    try {
      await fetch(`/api/deals/${dealId}`, {
        ...
        body: JSON.stringify({ firm_type: selectedFirm, aum: selectedAum || undefined, rerun }),
      });
...
function DealDashboard() {
    ...
    return (
      <div className="widget deal-pipe">
        <div className="pipe-header">
          <div className="pipe-title">Deal Not Found</div>
        </div>
        <button className="cp-btn-process" onClick={() => sendFollowUp("Create a new deal analysis")}>
          Create New Deal
        </button>
      </div>
    );
  ...
  return (
    <div className="widget deal-pipe" data-llm={`Deal: ${d.deal_input?.name}, Decision: ${dg?.decision || "pending"}, Avg: ${avgScore || "?"}, Analysts: ${analystsDone}/${pl.analysts.length}`}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          <button
            ...
            onClick={async () => {
              ...
              setRetrying(true);
              try {
                await fetch(`/api/deals/${dealId}/run`, { method: "POST" });
                // Wait a beat then start polling
                setTimeout(() => refresh({ deal_id: dealId }), 2000);
          ...
          <button className="action-btn" onClick={() => refresh({ deal_id: (input as any)?.deal_id || "" })} disabled={isRefreshing}>
      ...
      <InvestorProfileBadge
        ...
        dealId={d.deal_id || input?.deal_id || ""}
        onSwitch={(newFirmType, newAum) => {
          ...
          refresh({ deal_id: d.deal_id || input?.deal_id || "" });
        }}
      />
      ...
      {/* ═══ COMPETITIVE INTELLIGENCE (Specter + targeted resolution) ═══ */}
      {(() => {
        const resolveFeed = liveUpdates.filter(u =>
          u.phase.startsWith("resolve_") || u.phase.startsWith("competitive_") ||
          u.phase === "unknown_resolution" || u.phase === "unknown_resolution_done"
        );
        if (resolveFeed.length === 0) return <div className="pipe-connector" />;
        const isDone = resolveFeed.some(u => u.phase === "unknown_resolution_done" || u.phase === "resolve_specter_done");
        const specterItems = resolveFeed.filter(u => u.phase.includes("specter") || u.phase.includes("competitor"));
        const resolvedCount = resolveFeed.filter(u => u.text.startsWith("✅")).length;
        const totalSteps = resolveFeed.filter(u =>
          (u.phase.startsWith("resolve_") && !u.phase.endsWith("_done")) ||
          u.phase === "competitive_intel"
        ).length;
        return (
          <>
            <div className="pipe-connector" />
            <details className={`toggle-node ${isDone ? "node-done" : "node-running"}`} open={!isDone}>
              <summary className="toggle-summary">
                <span className={`agent-status ${isDone ? "node-done" : "node-running"}`}>{isDone ? "✓" : "●"}</span>
                <span className="toggle-role">Competitive Intelligence</span>
                {isDone
                  ? <span className="toggle-stats">{resolvedCount} resolved · {specterItems.length > 0 ? "Specter" : "Tavily"}</span>
                  : <span className="toggle-stats pulse-text">enriching competitors…</span>
                }
              </summary>
              <div className="toggle-feed">
                {resolveFeed.map((u, i) => {
                  const isQuery = (u.phase.startsWith("resolve_") && !u.phase.endsWith("_done")) ||
                    u.phase === "competitive_intel";
                  const matchingDone = isQuery && resolveFeed.some(d => d.phase === `${u.phase}_done`);
                  const isSpecter = u.phase.includes("specter") || u.phase.includes("competitor");
                  return (
                    <div key={i} className={`feed-item ${
                      isQuery
                        ? (matchingDone ? "feed-done" : "feed-resolve")
                        : u.text.startsWith("✅") || u.text.startsWith("✓")
                          ? "feed-done"
                          : "feed-resolve"
                    }${isQuery && !matchingDone && !isDone ? " feed-active" : ""}${isSpecter ? " feed-specter" : ""}`}>
                      {u.text}
                    </div>
                  );
                })}
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        {!isComplete && (
          <button className="action-btn action-btn-primary" onClick={() => refresh({ deal_id: input?.deal_id || "" })} disabled={isRefreshing}>
